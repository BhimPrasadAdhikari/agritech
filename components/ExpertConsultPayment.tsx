...
const ExpertConsultPayment = ({ expert }: { expert:  User & { image: ImageType | null }}) => {
  const [loading, setLoading] = useState(false);
  const use_expert = useExpert();

  const total_amount = expert.price || 0;
  const handlePayment = async (paymentMethod: string) => {
    setLoading(true);
    use_expert.addExpert(expert);
  ...
  return (
    <div className="w-full">
      <h3 className="text-xl font-bold mb-4">
        Confirm Consultation with {expert.name}
      </h3>
      ...
      <div className="border-t-2">
        <div className="mt-2">
          ...
          >
            ...
            <motion.div className="relative w-24 h-24 rounded-full overflow-visible mb-4">
              <motion.div
                className="overflow-hidden rounded-full w-24 h-24"
                whileHover={{
                  scale: 1.2, // Scale up the image
                }}
                transition={{ type: "spring", stiffness: 200, damping: 10 }}
              >
                <Image
                  src={expert.image?.url || "/images/profile.png"}
                  alt={expert.name || "profile"}
                  className="rounded-full mb-4 object-cover"
                  fill
                  priority // Optional: prioritize this image for loading
                />
              </motion.div>
            </motion.div>
            ...
            <p className="text-gray-600">Available: {expert.availability}</p>
            <p className="mt-4 text-yellow-500">Rating: {expert.rating} ⭐</p>
          </motion.div>
          <div className="mt-2">
            <p className="mb-4">Price: ${expert.price}</p>
            <div>
              <Button
                ...
                {loading ? (
                  ...
                  >
                    <g id="khalti-logo-01" transform="translate(0)">
                      ...
                      <g
                        id="XMLID_432_"
                        transform="translate(0 7.85) rotate(-6)"
                      >
                        <g
                          id="XMLID_453_-link"
...
export default ExpertConsultPayment;

// alert("Payment successful! Consultation booked.");
// ExpertConsultPaymentModal.onClose(); // Close payment modal
