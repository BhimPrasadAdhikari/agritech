...
const PredictPage = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto flex flex-col gap-6 p-6 bg-green-50 rounded-lg shadow-lg">
      <h1 className="font-bold text-3xl text-green-700">Plant Disease Detection</h1>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        />
        <Button type="submit" className="bg-green-500 text-white py-2 rounded-md hover:bg-green-600 transition-all">Submit</Button>
      </form>
      ...
      {prediction && (
        ...
        >
          ...
          <div>
            ...
            <p>{diseaseInfo[prediction]?.prevention || "No information available"}</p>
          </div>

          <div className="mt-4">
            <h3 className="font-bold text-green-600">Recommended Fertilizers:</h3>
            <p>{diseaseInfo[prediction]?.fertilizers || "No information available"}</p>
          </div>

          <div className="mt-4">
            <h3 className="font-bold text-green-600">Expert Advice:</h3>
            <p>{diseaseInfo[prediction]?.expertAdvice || "No information available"}</p>
          </div>
        </motion.div>
      )}
    </div>
